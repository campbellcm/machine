...
function ChannelIcon({ channel }: { channel: "linkedin" | "x" }) {
  ...
  return (
    <span
      className={`account-icon ${channel}`}
      role="img"
      aria-label={name}
      title={name}
    >
      {channel === "linkedin" ? (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path
            fill="currentColor"
            d="M5.4 7.8H2V22h3.4V7.8ZM3.7 2a2 2 0 1 0 0 4 2 2 0 0 0 0-4ZM22 13.8c0-4.3-2.3-6.3-5.3-6.3-2.5 0-3.6 1.4-4.2 2.3v-2H9.1V22h3.4v-7.9c0-2.1.4-4.1 3-4.1 2.6 0 2.6 2.4 2.6 4.2V22H22v-8.2Z"
          />
        </svg>
      ) : (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path
            fill="currentColor"
            d="M18.9 2H22l-6.8 7.8L23.2 22h-6.3L12 14.6 5.5 22H2.3l8.2-9.4L.8 2h6.5l4.5 6.8L18.9 2Zm-1.1 18h1.7L6.3 4H4.5l13.3 16Z"
          />
        </svg>
      )}
    </span>
            ...
            <section className="v1-card" key={key}>
              ...
              {previous && (
                <small>
                  {changeLabel(
                    total,
                    previous.people.length &&
                      previous.people.every((p) => p[key] !== null)
                      ? previous.people.reduce(
                          (sum, p) => sum + (p[key] ?? 0),
                          0,
                        )
                      : null,
        ...
        <div className="feed-filters">
          ...
          <label>
            ...
            >
              ...
              {Array.from(
                new Map(
                  report.posts.map((p) => [p.user_id, p.author]),
                ).entries(),
              )
          ...
          <article className="v1-card v1-post" key={p.id}>
            <div className="v1-section-title">
              <div className="feed-author">
                <TeamAvatar
                  name={p.author || "Teammate"}
                  src={
                    report.people.find((person) => person.id === p.user_id)
                      ?.photo_url
                  }
                />
            ...
            <p id={`post-${p.id}`} className="feed-body">
              {expanded.has(p.id) || p.body.length <= 180
                ? p.body
                : p.body.slice(0, 180).trimEnd() + "…"}
            </p>
            {p.body.length > 180 && (
              <button
                className="feed-expand"
                aria-expanded={expanded.has(p.id)}
                aria-controls={`post-${p.id}`}
                onClick={() =>
              ...
              >
                {expanded.has(p.id) ? "See less" : "See more"}
                <span className="sr-only"> of {p.author}’s post</span>
              </button>
            )}
            {p.url &&
