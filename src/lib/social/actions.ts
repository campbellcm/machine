...
export async function disconnectLinkedIn() {
  ...
  const { error } = await db.rpc("disconnect_social");
  redirect(
...
export async function publishLinkedIn(f: FormData) {
  ...
  const { data: claimed, error } = await db.rpc("claim_publish", {
    draft: id,
  ...
  try {
    const token = decryptToken(
      ...
      `${org.id}:${user.id}:linkedin`,
    );
    const result = await sendLinkedIn(token, account.provider_user_id, claimed);
    if (result.ok) {
      const { error: saveError } = await service.rpc("complete_publish", {
        draft: id,
        post_id: result.id,
      });
